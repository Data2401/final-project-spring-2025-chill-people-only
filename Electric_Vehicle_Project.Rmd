...
county_EV <- ev_data %>% group_by(County) %>% summarize(
  ...
  ggplot(county_EV, aes(x = Total_EV,y=County )) +
...
model_year_trends <- ev_data %>%
  ...
  arrange(Model.Year)
ggplot(model_year_trends, aes(x = Model.Year, y = Total_Registrations)) +
...
### Is there a correlation between electric utility providers and EV adoption rates?
```
###
